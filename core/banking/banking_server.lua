    ...
    TriggerEvent('sarp:getPlayerFromId', source, function(user)
        ...
        if(string.len(rounded) >= 9) then
        	...
        	if(tonumber(rounded) <= tonumber(user:money)) then
                ...
                TriggerClientEvent("sarp:notify", source, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Deposited: ~g~$".. rounded .." ~n~~s~New Balance: ~g~$" .. new_balance)
    ...
    TriggerEvent('sarp:getPlayerFromId', source, function(user)
        ...
        if(string.len(rounded) >= 9) then
            ...
            if(tonumber(rounded) <= tonumber(bankbalance)) then
                ...
                local new_balance = bankBalance(player)
                TriggerClientEvent("sarp:notify", source, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Withdrew: ~g~$".. rounded .." ~n~~s~New Balance: ~g~$" .. new_balance)
                TriggerClientEvent("banking:updateBalance", source, new_balance)
            ...
            if(string.len(rounded) >= 9) then
                ...
                if(tonumber(rounded) <= tonumber(bankbalance)) then
                    ...
                    TriggerEvent('sarp:getPlayerFromId', toPlayer, function(user2)
                        ...
                        TriggerClientEvent("sarp:notify", toPlayer, "CHAR_BANK_MAZE", 1, "Maze Bank", false, "Received: ~g~$".. rounded .." ~n~~s~New Balance: ~g~$" .. new_balance2)
